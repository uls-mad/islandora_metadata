<xsl:stylesheet xmlns="http://www.loc.gov/mods/v3" xmlns:marc="http://www.loc.gov/MARC21/slim" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" exclude-result-prefixes="xlink marc" version="1.0">
	...
	MARC21slim2MODS3-8_XSLT1-0
	
	MODS 3.8  (Revision 1.174) 20250328

	Revision 1.174 - digitalOrigin add born digital, 506$u and 540u						2025/03/28 ntra	
	Revision 1.173 - 005 insert "T" between date and time for ISO-8601:2019 compliance.    2024/10/03 ntra
	Revision 1.172 - 033 remove iso encoding. 2023/02/08 ws
	Revision 1.171 - Change roleTerm to lowercase to match marcrelator list. 2023/01/25 ws
	Revision 1.170 - 264_3 remove duplicate. 2023/01/25 ws
	Revision 1.169 - 510 $c make part element conditional. 2023/01/25 ws
	Revision 1.168 - 776 $d Updated to match mapping. 2023/01/25 ws
	Revision 1.167 - 830 $p add partName. 2023/01/25 ws  
	Revision 1.166 - Fix Bug in code incorrectly suppressing agent display in orginInfo. 2023/01/25 ws
	Revision 1.165 - 008/28 BK CF CR MP VM remove |. 2023/01/17 ws
	Revision 1.164 - 037 $c Comment out displayLabel. 2023/01/17 ws 
	Revision 1.163 - 044 $c Fixed text appearing outside of the element. 2023/01/17 ws
	Revision 1.162 - 730 _2 $t add subfield $t to display. 2023/01/17 ws
	Revision 1.161 - 510 $a with second indicator other than 0 missing from output, as is content of 510x. 2023/01/17 ws
	Revision 1.160 - 653 add a for each statement so each subfield ouputs as its own element. 2023/01/17 ws
	Revision 1.159 - 264 _4 $c Correct mapping for orginInfo dates. 2023/01/17 ws
	Revision 1.158 - 880-630 add subfield t to output. 2023/01/17 ws 
	Revision 1.157 - Fix parsing error. 2023/01/17 ws 
	Revision 1.156 - 880 245 c Correct altRepGroup output. 2023/01/17 ws
	Revision 1.155 - 880 correct mapping for script code in 880 fields. 2023/01/17 ws
	Revision 1.154 - 520 $c add subfield c. 2023/01/17 ws
	Revision 1.153 - 264_1 namePart Change to Publisher instead of publisher. 2023/01/17 ws 
	Revision 1.152 - 264_1 Change publisher/producer/distributor/manufacturer to nouns, i.e. production, publication, distribution, manufacture. 2023/01/17 ws
	Revision 1.151 - 653 change indicator rule form @ind to @ind1 and correct mapping. 2023/01/17 ws
	
	Revision 1.150 - 334 $a add originInfo/issuance. 2022/09/20 ws
	Revision 1.149 - 534 $c, 760 762, 765, 767, 770, 772, 773, 774, 775, 776, 777, 780, 785, 786, 787 $d relatedItem/note and @type="publication information". 2022/09/20 ws
	Revision 1.148 - 506, 540  $2 with authority="contents of subfield". 2022/09/20 ws
	Revision 1.147 - 043 $2 subject/geographicCode with authority="contents of $2". 2022/09/20 ws
	Revision 1.146 - 505 ind1 = 0 <tableOfContents> with attribute @type="Contents", 505 ind1 = 1 <tableOfContents> with attribute @type="Incomplete contents", 505 ind1 = 2	<tableOfContents> with attribute @type="Partial contents". 2022/09/20 ws 
	Revision 1.145 - 520 ind1 = # (blank) <abstract> with attribute @type="Summary", 520 ind1 = 0 <abstract> with attribute @type="Subject", 520 ind1 = 1	<abstract> with attribute @type="Review", 520 ind1 = 2	<abstract> with attribute @type="Scope and content", 520 ind1 = 3	<abstract> with attribute @type="Abstract", 520 ind1 = 4	<abstract> with attribute @type="Content advice". 2022/09/20 ws
	Revision 1.144 - 260 $f originInfo/agent/namePart with role/roleTerm = 'Manufacturer'. 2022/09/20 ws
	Revision 1.143 - 260 $b, 264 $b originInfo/agent/namePart Use without role term. 2022/09/20 ws
	Revision 1.142 - 100,700,110,710, 111, 711, 600 $1 Add name/nameIdentifier with contents of $1. 2022/09/20 ws
	Revision 1.141 - For fields 100, 700, 110, 710, 111, 711, 336, 655, 337, 338, 600, 610, 611 with $0 add valueURI, only if contents are a URI. 2022/09/20 ws 
	...
	<xsl:template match="/">
		<xsl:choose>
			<xsl:when test="//marc:collection">
				<modsCollection xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.loc.gov/mods/v3 http://www.loc.gov/standards/mods/v3/mods-3-8.xsd">
			...
			<xsl:otherwise>
				<mods xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" version="3.8" xsi:schemaLocation="http://www.loc.gov/mods/v3 http://www.loc.gov/standards/mods/v3/mods-3-8.xsd">
...
-->
		...
		<xsl:for-each select="marc:datafield[@tag=264][@ind2=0] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'264')][@ind2=0]">
			...
			<originInfo eventType="production">
						...
						<xsl:for-each select="marc:subfield[@code='a']">
							...
							<xsl:if test="following-sibling::marc:subfield[@code='b']">
								<xsl:for-each select="following-sibling::marc:subfield[@code='b'][1]">
									<!-- 1.144 -->
									<agent><namePart><xsl:value-of select="."/></namePart></agent>
								</xsl:for-each>
							</xsl:if>
						</xsl:for-each>
					</xsl:when>
				...
				<!-- 1.150 -->
				<xsl:if test="../marc:datafield[@tag=334][marc:subfield[@code='a']]">
					<issuance><xsl:value-of select="../marc:datafield[@tag=334]/marc:subfield[@code='a']"/></issuance>
				</xsl:if>
			</originInfo>
		</xsl:for-each>
		<!-- 1.121 -->
							...
							<xsl:if test="following-sibling::marc:subfield[@code='b']">
								<xsl:for-each select="following-sibling::marc:subfield[@code='b'][1]">
									...
									<agent>
										...
										<!-- 1.171 -->
										<role><roleTerm>publisher</roleTerm></role>
									</agent>
								</xsl:for-each>
							</xsl:if>
						</xsl:for-each>
							...
							<xsl:if test="following-sibling::marc:subfield[@code='b']">
								<xsl:for-each select="following-sibling::marc:subfield[@code='b'][1]">
									...
									<agent>
										<namePart><xsl:value-of select="."/></namePart>
										<!-- 1.171 -->
										<role><roleTerm>distributor</roleTerm></role>
									</agent>
								</xsl:for-each>
							</xsl:if>
						</xsl:for-each>
					...
					<xsl:otherwise>
						...
						<xsl:for-each select="marc:subfield[@code='b']">
							...
							<agent>
								<namePart><xsl:value-of select="."/></namePart>
								<!-- 1.171 -->
								<role><roleTerm>manufacturer</roleTerm></role>
							</agent>
						</xsl:for-each>
					</xsl:otherwise>
				</xsl:choose>
...
-->
		...
		<xsl:variable name="physicalDescription">
			...
			<xsl:apply-templates
				...
				| marc:datafield[@tag='256']/marc:subfield[@code='a'] | marc:datafield[@tag='337']/marc:subfield[@code='a'] 
		...
		<!-- WS:Note add 340/344 -->
		<xsl:for-each select="marc:datafield[@tag='340'] | marc:datafield[@tag='344']">
			<xsl:apply-templates select="." mode="physDesc"/>
		</xsl:for-each>
		<!-- WS:Note add 340/344 -->
		...
		<xsl:for-each select="marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'130')]/child::*[@code='h'] | 
			...
			| marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'340')] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'344')]">
			<physicalDescription>
				<!-- 1.141 -->
				<xsl:if test="self::marc:datafield[@tag='337'] or self::marc:datafield[@tag='338']">
		...
		<!-- 1.161 -->
		<xsl:for-each select="marc:datafield[@tag=510] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'510')]">
			<xsl:variable name="s6" select="substring(normalize-space(marc:subfield[@code='6']), 5, 2)"/>
			<!-- 1.161 -->
			<xsl:if test="@tag=510 or (@tag='880' and not(../marc:datafield[@tag='510'][substring(marc:subfield[@code='6'],5,2) = $s6]))">
		...
		<xsl:for-each select="marc:datafield[@tag=534][@ind1='0' or @ind1=' '] | marc:datafield[@tag='880'][@ind1='0' or @ind1=' '][starts-with(marc:subfield[@code='6'],'534')]">
			...
			<relatedItem type="original">
				<xsl:for-each
					...
					<!-- 1.149 -->
					<xsl:for-each select="marc:subfield[@code='c']">
						<note type="publication information">
							<xsl:value-of select="."/>
						</note>
					</xsl:for-each>
				</xsl:for-each>
							...
							<title>
								<xsl:call-template name="chopPunctuation">
									<xsl:with-param name="chopString">
										<!-- 1.120 @711$v 1.162 -->
										<xsl:call-template name="subfieldSelect">
											<xsl:with-param name="codes">adfgklmorst</xsl:with-param>
										</xsl:call-template>
									</xsl:with-param>
								</xsl:call-template>
							</title>
		...
		<xsl:if test="child::marc:subfield[@code='6']">
			...
			<xsl:choose>
				...
				<xsl:when test="$scriptCode != '' and $scriptCode != ' '">
						<xsl:choose>
							<xsl:when test="$scriptCode='(3'"><xsl:attribute name="script">Arab</xsl:attribute></xsl:when>
							<xsl:when test="$scriptCode='(4'"><xsl:attribute name="script">>Arab</xsl:attribute></xsl:when>
							<xsl:when test="$scriptCode='(B'"><xsl:attribute name="script">Latn</xsl:attribute></xsl:when>
							<xsl:when test="$scriptCode='!E'"><xsl:attribute name="script">Latn</xsl:attribute></xsl:when>
							<xsl:when test="$scriptCode='$1'"><xsl:attribute name="script">CJK</xsl:attribute></xsl:when>
							<xsl:when test="$scriptCode='(N'"><xsl:attribute name="script">Cyrl</xsl:attribute></xsl:when>
							<xsl:when test="$scriptCode='(Q'"><xsl:attribute name="script">Cyrl</xsl:attribute></xsl:when>
							<xsl:when test="$scriptCode='(2'"><xsl:attribute name="script">Hebr</xsl:attribute></xsl:when>
							<xsl:when test="$scriptCode='(S'"><xsl:attribute name="script">Grek</xsl:attribute></xsl:when>
						</xsl:choose>
				</xsl:when>
			</xsl:choose>
		</xsl:if>
	...
	<xsl:template name="createTitleInfoFrom730">
		<titleInfo type="uniform">
			...
			<title>
				<xsl:variable name="str">
					<xsl:call-template name="subfieldSelect">
						<xsl:with-param name="codes">adfklmorst</xsl:with-param>
					</xsl:call-template>
				</xsl:variable>
				<xsl:call-template name="chopPunctuation">
					<xsl:with-param name="chopString">
		...
		<genre authority="marcgt">
			...
			<xsl:choose>
				...
				<xsl:when test="@ind2 = 0">
					<xsl:attribute name="authority"><xsl:text>Library of Congress Subject Headings</xsl:text></xsl:attribute>
				</xsl:when>
				<xsl:when test="@ind2 = 1">
					<xsl:attribute name="authority"><xsl:text>LC subject headings for children's literature</xsl:text></xsl:attribute>
				</xsl:when>
				<xsl:when test="@ind2 = 2">
					<xsl:attribute name="authority"><xsl:text>Medical Subject Headings</xsl:text></xsl:attribute>
				</xsl:when>
				<xsl:when test="@ind2 = 3">
					<xsl:attribute name="authority"><xsl:text>National Agricultural Library subject authority file</xsl:text></xsl:attribute>
				</xsl:when>
				<xsl:when test="@ind2 = 4">
					<xsl:attribute name="authority"><xsl:text>Source not specified</xsl:text></xsl:attribute>
				</xsl:when>
				<xsl:when test="@ind2 = 5">
					<xsl:attribute name="authority"><xsl:text>Canadian Subject Headings</xsl:text></xsl:attribute>
				</xsl:when>
				<xsl:when test="@ind2 = 6">
					<xsl:attribute name="authority"><xsl:text>Répertoire de vedettes-matière</xsl:text></xsl:attribute>
				</xsl:when>
			</xsl:choose>
			<!-- Template checks for altRepGroup - 880 $6 -->
			<xsl:call-template name="xxx880"/>
		...
		<tableOfContents>
			...
			<xsl:choose>
				<xsl:when test="@ind1='0'">
					<xsl:attribute name="type">Contents</xsl:attribute>
				</xsl:when>
				<xsl:when test="@ind1='1'">
					<xsl:attribute name="type">Incomplete contents</xsl:attribute>
				</xsl:when>
				...
				<note type="statement of responsibility">
					...
					<xsl:choose>
						<xsl:when test="@tag = 880">
							<xsl:if test="../marc:datafield[@tag='245'][marc:subfield[@code='c']]"><xsl:call-template name="xxx880"/></xsl:if>
						</xsl:when>
						<xsl:when test="@tag = 245">
							<xsl:if test="../marc:datafield[@tag='880'][marc:subfield[@code='c']]"><xsl:call-template name="xxx880"/></xsl:if>
						</xsl:when>
					</xsl:choose>
					<xsl:call-template name="subfieldSelect">
						<xsl:with-param name="codes">c</xsl:with-param>
		...
		<subject>
			...
			<xsl:for-each select="marc:subfield[@code='a' or @code='b' or @code='c']">
				<geographicCode>
					<!-- 1.147 -->
					<xsl:choose>
						<xsl:when test="@code='a'"><xsl:attribute name="authority">marcgac</xsl:attribute></xsl:when>
						<xsl:when test="@code='c'"><xsl:attribute name="authority">iso3166</xsl:attribute></xsl:when>
						<!-- 1.147 -->
						<xsl:when test="@code='b' and following-sibling::marc:subfield[@code='2']"><xsl:attribute name="authority"><xsl:value-of select="following-sibling::marc:subfield[@code='2']"/></xsl:attribute></xsl:when>
					</xsl:choose>
...
<!--			<xsl:apply-templates select="marc:subfield[@code='0']" mode="xlink"/>-->
	...
	<xsl:template name="createAccessConditionFrom506">
		<accessCondition type="restriction on access">
			...
			<!-- 1.174 506u -->
			<xsl:if test="marc:subfield[@code='u']">
				<xsl:attribute name="valueURI"><xsl:value-of select="marc:subfield[@code='u']"/></xsl:attribute>
			</xsl:if>
			<xsl:call-template name="xxx880"/>
			<xsl:call-template name="subfieldSelect">
	...
	<!-- WS:note add 340/344 -->
	<xsl:template
		match="marc:datafield[@tag='340'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'340')]"
		mode="physDesc">
		<xsl:variable name="content">
			<xsl:for-each select="marc:subfield">
				<xsl:choose>
					<xsl:when test="@code='a'">
						<form type="Material base and configuration">
							<xsl:if test="marc:subfield[@code='0']"><xsl:attribute name="valueURI"><xsl:value-of select="marc:subfield[@code='0']"/></xsl:attribute></xsl:if>
							<xsl:if test="marc:subfield[@code='2']"><xsl:attribute name="authority"><xsl:value-of select="marc:subfield[@code='2']"/></xsl:attribute></xsl:if>
							<xsl:value-of select="."/>
						</form>
					</xsl:when>
					<xsl:when test="@code='b'">
					...
					<xsl:when test="@code='c'">
						<form type="Materials applied to surface">
							<xsl:if test="marc:subfield[@code='0']"><xsl:attribute name="valueURI"><xsl:value-of select="marc:subfield[@code='0']"/></xsl:attribute></xsl:if>
							<xsl:if test="marc:subfield[@code='2']"><xsl:attribute name="authority"><xsl:value-of select="marc:subfield[@code='2']"/></xsl:attribute></xsl:if>
							<xsl:value-of select="."/></form>
					</xsl:when>
					<xsl:when test="@code='d'">
						<form type="Information recording technique">
							<xsl:if test="marc:subfield[@code='0']"><xsl:attribute name="valueURI"><xsl:value-of select="marc:subfield[@code='0']"/></xsl:attribute></xsl:if>
							<xsl:if test="marc:subfield[@code='2']"><xsl:attribute name="authority"><xsl:value-of select="marc:subfield[@code='2']"/></xsl:attribute></xsl:if>
							<xsl:value-of select="."/></form>
					</xsl:when>
					...
					<xsl:when test="@code='f'">
						<note displayLabel="Reduction ratio value">
							<xsl:if test="marc:subfield[@code='3']"><xsl:attribute name="type"><xsl:value-of select="marc:subfield[@code='3']"/>Materials specified</xsl:attribute></xsl:if>
							<xsl:value-of select="."/></note>
					</xsl:when>
					<xsl:when test="@code='g'">
						<form type="Color content">
							<xsl:if test="marc:subfield[@code='0']"><xsl:attribute name="valueURI"><xsl:value-of select="marc:subfield[@code='0']"/></xsl:attribute></xsl:if>
							<xsl:if test="marc:subfield[@code='2']"><xsl:attribute name="authority"><xsl:value-of select="marc:subfield[@code='2']"/></xsl:attribute></xsl:if>
							<xsl:value-of select="."/></form>
					</xsl:when>
					<xsl:when test="@code='h'">
						<note displayLabel="Location within medium">
							<xsl:if test="marc:subfield[@code='3']"><xsl:attribute name="type"><xsl:value-of select="marc:subfield[@code='3']"/>Materials specified</xsl:attribute></xsl:if>
							<xsl:value-of select="."/></note>
					</xsl:when>
					<xsl:when test="@code='i'">
						<note displayLabel="Technical specifications of medium">
							<xsl:if test="marc:subfield[@code='3']"><xsl:attribute name="type"><xsl:value-of select="marc:subfield[@code='3']"/>Materials specified</xsl:attribute></xsl:if>
							<xsl:value-of select="."/></note>
					</xsl:when>
					...
					<xsl:when test="@code='p'">
						<form type="Illustrative content">
							<xsl:if test="marc:subfield[@code='0']"><xsl:attribute name="valueURI"><xsl:value-of select="marc:subfield[@code='0']"/></xsl:attribute></xsl:if>
							<xsl:if test="marc:subfield[@code='2']"><xsl:attribute name="authority"><xsl:value-of select="marc:subfield[@code='2']"/></xsl:attribute></xsl:if>
							<xsl:value-of select="."/></form>
					</xsl:when>
					<xsl:when test="@code='q'">
						<form type="Reduction ratio designator">
							<xsl:if test="marc:subfield[@code='0']"><xsl:attribute name="valueURI"><xsl:value-of select="marc:subfield[@code='0']"/></xsl:attribute></xsl:if>
							<xsl:if test="marc:subfield[@code='2']"><xsl:attribute name="authority"><xsl:value-of select="marc:subfield[@code='2']"/></xsl:attribute></xsl:if>
							<xsl:value-of select="."/></form>
					</xsl:when>
	...
	<xsl:template
		match="marc:datafield[@tag='344'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'344')]"
		mode="physDesc">
		<xsl:variable name="content">
			<xsl:for-each select="marc:subfield">
				<xsl:choose>
					<xsl:when test="@code='a'">
						<form type="Type of recording">
							<xsl:if test="marc:subfield[@code='0']"><xsl:attribute name="valueURI"><xsl:value-of select="marc:subfield[@code='0']"/></xsl:attribute></xsl:if>
							<xsl:if test="marc:subfield[@code='2']"><xsl:attribute name="authority"><xsl:value-of select="marc:subfield[@code='2']"/></xsl:attribute></xsl:if>
							<xsl:value-of select="."/></form>
					</xsl:when>
					<xsl:when test="@code='b'">
						<form type="Recording medium">
							<xsl:if test="marc:subfield[@code='0']"><xsl:attribute name="valueURI"><xsl:value-of select="marc:subfield[@code='0']"/></xsl:attribute></xsl:if>
							<xsl:if test="marc:subfield[@code='2']"><xsl:attribute name="authority"><xsl:value-of select="marc:subfield[@code='2']"/></xsl:attribute></xsl:if>
							<xsl:value-of select="."/></form>
					</xsl:when>
					<xsl:when test="@code='c'">
						<form type="Playing speed">
							<xsl:if test="marc:subfield[@code='0']"><xsl:attribute name="valueURI"><xsl:value-of select="marc:subfield[@code='0']"/></xsl:attribute></xsl:if>
							<xsl:if test="marc:subfield[@code='2']"><xsl:attribute name="authority"><xsl:value-of select="marc:subfield[@code='2']"/></xsl:attribute></xsl:if>
							<xsl:value-of select="."/></form>
					</xsl:when>
					<xsl:when test="@code='d'">
						<form type="Groove characteristic">
							<xsl:if test="marc:subfield[@code='0']"><xsl:attribute name="valueURI"><xsl:value-of select="marc:subfield[@code='0']"/></xsl:attribute></xsl:if>
							<xsl:if test="marc:subfield[@code='2']"><xsl:attribute name="authority"><xsl:value-of select="marc:subfield[@code='2']"/></xsl:attribute></xsl:if>
							<xsl:value-of select="."/></form>
					</xsl:when>
					<xsl:when test="@code='e'">
						<form type="Track configuration">
							<xsl:if test="marc:subfield[@code='0']"><xsl:attribute name="valueURI"><xsl:value-of select="marc:subfield[@code='0']"/></xsl:attribute></xsl:if>
							<xsl:if test="marc:subfield[@code='2']"><xsl:attribute name="authority"><xsl:value-of select="marc:subfield[@code='2']"/></xsl:attribute></xsl:if>
							<xsl:value-of select="."/></form>
					</xsl:when>
					<xsl:when test="@code='f'">
						<form displayLabel="Tape configuration">
							<xsl:if test="marc:subfield[@code='0']"><xsl:attribute name="valueURI"><xsl:value-of select="marc:subfield[@code='0']"/></xsl:attribute></xsl:if>
							<xsl:if test="marc:subfield[@code='2']"><xsl:attribute name="authority"><xsl:value-of select="marc:subfield[@code='2']"/></xsl:attribute></xsl:if>
							<xsl:value-of select="."/></form>
					</xsl:when>
					<xsl:when test="@code='g'">
						<form type="Configuration of playback channels">
							<xsl:if test="marc:subfield[@code='0']"><xsl:attribute name="valueURI"><xsl:value-of select="marc:subfield[@code='0']"/></xsl:attribute></xsl:if>
							<xsl:if test="marc:subfield[@code='2']"><xsl:attribute name="authority"><xsl:value-of select="marc:subfield[@code='2']"/></xsl:attribute></xsl:if>
							<xsl:value-of select="."/></form>
					</xsl:when>
					<xsl:when test="@code='h'">
						<form type="Special playback characteristics">
							<xsl:if test="marc:subfield[@code='0']"><xsl:attribute name="valueURI"><xsl:value-of select="marc:subfield[@code='0']"/></xsl:attribute></xsl:if>
							<xsl:if test="marc:subfield[@code='2']"><xsl:attribute name="authority"><xsl:value-of select="marc:subfield[@code='2']"/></xsl:attribute></xsl:if>
							<xsl:value-of select="."/></form>
					</xsl:when>
					<xsl:when test="@code='i'">
						<form type="Sound content">
							<xsl:if test="marc:subfield[@code='0']"><xsl:attribute name="valueURI"><xsl:value-of select="marc:subfield[@code='0']"/></xsl:attribute></xsl:if>
							<xsl:if test="marc:subfield[@code='2']"><xsl:attribute name="authority"><xsl:value-of select="marc:subfield[@code='2']"/></xsl:attribute></xsl:if>
							<xsl:value-of select="."/></form>
					</xsl:when>
					<xsl:when test="@code='j'">
						<form type="Original capture and storage technique">
							<xsl:if test="marc:subfield[@code='0']"><xsl:attribute name="valueURI"><xsl:value-of select="marc:subfield[@code='0']"/></xsl:attribute></xsl:if>
							<xsl:if test="marc:subfield[@code='2']"><xsl:attribute name="authority"><xsl:value-of select="marc:subfield[@code='2']"/></xsl:attribute></xsl:if>
							<xsl:value-of select="."/></form>
					</xsl:when>
	...
	<xsl:template name="digitalOrigin">
		...
		<!--1.174 born digital-->		
		<xsl:if test="$typeOf008='CF' and marc:controlfield[@tag='007'][substring(.,12,1)='n']">
	...
	<xsl:template name="originInfo">
		...
		<xsl:variable name="originInfoShared">
			...
			<!-- 1.159  -->
			<xsl:if test="$controlField008-6!='t' and ../marc:datafield[@tag=264][@ind2=4][marc:subfield[@code='c']]">
				<dateIssued encoding="marc">
					<xsl:value-of select="../marc:datafield[@tag=264][@ind2=4][marc:subfield[@code='c']]"/>
				</dateIssued>
			</xsl:if>
			<xsl:for-each select="marc:leader">
				<!-- 1.120 - @260$issuance -->
